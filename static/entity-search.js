...
window.EntitySearch = (function () {
  ...
  function injectStyle() {
    ...
    var css = '' +
      ...
      '.es-group-title{font-size:0.7rem;color:var(--accent);padding:0.25rem 0.3rem;font-weight:600}' +
  ...
  function internalHtml(data, query, opts) {
    ...
    order.forEach(function (kind) {
      var meta = kinds[kind] || {};
      html += '<div class="es-group"><div class="es-group-title">' +
        esc(meta.icon || '•') + ' ' + esc(meta.label || kind) +
        ' <span class="es-count">' + groups[kind].length + '</span></div>';
      groups[kind].forEach(function (row) {
        html += '<div class="es-row" role="button" tabindex="0" data-kind="' + esc(row.kind) +
