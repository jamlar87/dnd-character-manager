...
def _merged_races_for_popup() -> dict:
    """RACES + manual races + alias keys — the lookup table the detail popup uses.

    Shared by the sheet route and the cached reference asset so both serve the
    same table (a private copy would drift).
    """
        ...
        if _target in merged_races:
            ...
            for _parent_name, _parent_data in merged_races.items():
                if _target in _parent_data.get("subraces", []):
                    _entry = {
                        "desc": _parent_data.get("subrace_descs", {}).get(_target, ""),
                        "source": _parent_data.get("_subrace_sources", {}).get(_target, ""),
                        "subrace": _target,
                    }
...
# ── Static sheet reference data → cacheable asset ───────────────────────────
# sheet.js needs the app's reference tables (classes, races, feats, summons,
# source slugs, ...). Inlining them put ~245 KB into EVERY sheet response for
# data that is identical for every character, so they are written once per
# process to static/sheet-ref.js and loaded ahead of sheet.js with a
# content-hash query. Only genuinely per-character values stay inline.
_SHEET_REF_VERSION: str | None = None


def _sheet_reference_payload() -> dict:
    """Character-independent tables consumed by static/sheet.js."""
    return {
        "classes": CLASSES,
        "races": _merged_races_for_popup(),
        "bgInfo": BACKGROUND_INFO,
        "summonTemplates": SUMMON_TEMPLATES,
        "sourceSlugMap": _get_source_slug_map(),
        "namedItemTypes": _get_named_item_types(),
        "featDetails": FEAT_DETAILS,
        "knownFeats": KNOWN_FEATS,
        "maneuverOptions": MANEUVER_OPTIONS,
        "skillAbMap": SKILL_ABILITIES,
    }
...
def ensure_sheet_reference_asset() -> str:
    """Write static/sheet-ref.js (once per process) and return its version hash.

    A failure here must not break the sheet: the page still renders, and the JS
    that needs those tables fails loudly in the console rather than silently
    serving stale data.
    """
    global _SHEET_REF_VERSION
    if _SHEET_REF_VERSION is not None:
        return _SHEET_REF_VERSION
    try:
        body = ("/* Generated by routes.characters.sheet.ensure_sheet_reference_asset(); "
                "character-independent data for /static/sheet.js. */\n"
                "window.SHEETREF = "
                + json.dumps(_sheet_reference_payload(), separators=(",", ":"), default=str)
                + ";\n")
        digest = hashlib.sha1(body.encode()).hexdigest()[:10]
        target = STATIC / "sheet-ref.js"
        if not target.exists() or target.read_text() != body:
            target.write_text(body)
        _SHEET_REF_VERSION = digest
    except Exception as exc:  # pragma: no cover - defensive
        print(f"[sheet-ref] generation failed: {exc}")
        _SHEET_REF_VERSION = "0"
    return _SHEET_REF_VERSION
...
async def character_sheet(char_id: int, request: Request):
    ...
    # (shared with the cached reference asset — see _merged_races_for_popup)
    merged_races = _merged_races_for_popup()

    return _render("sheet.html", request=request, character=char, spells=spells,
                   mi_spells_data=mi_spells_data,
                   dm_preview=dm_preview,
                   sheet_ref_version=ensure_sheet_reference_asset(),
                   sheet_js_version=sheet_asset_version("sheet.js"),
